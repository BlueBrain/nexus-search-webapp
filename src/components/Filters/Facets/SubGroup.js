...
const FacetSubgroup = (key, subFacet, onSelect) => {
  ...
  let defaultValue = visibleFacetOptions.filter(bucket => !!bucket.selected).map(bucket => bucket.key);
  return (
    <li key={key}>
      <FacetTitle title={key} total={total} numFilters={subFacet.buckets.length} />
      <FacetBody>
        <CheckboxGroup style={{ width: "100%" }} defaultValue={defaultValue} onChange={value => onSelect(key, value)}>
          {visibleFacetOptions.map(({ key:choiceKey, doc_count, selected }, index) => (
            <Facet
              key={`${choiceKey}-${index}`}
              label={choiceKey}
              value={choiceKey}
              selected={selected}
              amount={doc_count}
              onClick={value => onSelect(key, value)}
            />
          ))}
        </CheckboxGroup>
